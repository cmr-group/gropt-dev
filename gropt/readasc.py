...
Taken from pypulseq (https://github.com/imr-framework/pypulseq/), plus SAFE parameter helpers.
"""
...
def readasc(filename: str) -> Tuple[dict, dict]:
    """
    Read a Siemens ASC ascii-formatted text file into a dictionary structure.
    ...
    with open(filename, 'r') as fp:
        ...
        for next_line in fp:
            ...
            # Matches lines like 'a[0].b[2][3].c = "string" # comment'; assumes well-formed brackets
            match = re.match(
            ...
            if match:
                ...
                # value type from the matched regex group: string, int, float
                if match[3]:
...
def hw_namespace_to_dict(hw: SimpleNamespace) -> dict:
    ...
    ----------
    hw : SimpleNamespace
        Hardware parameters from :func:`asc_to_hw`.
    ...
    """
    out = {
        'tau1': np.array([hw.x.tau1, hw.y.tau1, hw.z.tau1]) / 1000.0,  # ms -> s
        'tau2': np.array([hw.x.tau2, hw.y.tau2, hw.z.tau2]) / 1000.0,
        'tau3': np.array([hw.x.tau3, hw.y.tau3, hw.z.tau3]) / 1000.0,
...
def asc_to_safe(asc_file: str) -> tuple[dict, dict]:
    """
    Read the SAFE PNS and cardiac model parameters from an ASC file.
    ...
    -------
    tuple[dict, dict]
        ``(params_pns, params_cns)``. ``params_cns`` is None if the file has no cardiac model;
        both are None (and an error is printed) if a required field is missing.
    """
...
def sample_between(
    N: int, low: float, high: float, rng: np.random.Generator, decimals: int | None = None
) -> np.ndarray:
    ...
    high : float
        Upper bound (exclusive, as in ``numpy.random.Generator.uniform``).
    rng : np.random.Generator
        Random number generator; seed it for reproducible results.
    decimals : int | None, optional
        Decimal places to round to; ``None`` for no rounding.

    Returns
    -------
    np.ndarray
        Array of shape ``(N,)`` containing sampled values.
...
def get_random_safe_params(rng_seed: int = 42) -> tuple[dict, dict]:
    """Generate random SAFE model parameters for PNS and cardiac models.

    Parameters
    ----------
    rng_seed : int, optional
        Seed for the random number generator. Default is 42.
        Use None for non-deterministic random parameters.

    Returns
    -------
    tuple[dict, dict]
        Two dictionaries: ``(params_pns, params_cns)``. Each dictionary
        contains axis-wise arrays for ``a1``, ``a2``, ``a3``, ``g_scale``,
        ``stim_limit``, ``tau1``, ``tau2``, and ``tau3``.
    """
    rng = np.random.default_rng(rng_seed)

    # PNS
    a1 = sample_between(3, 0.2, 0.3, rng, decimals=2)
    a2 = sample_between(3, 0.4, 0.6, rng, decimals=2)
    a3 = np.ones(3) - a1 - a2
    params_pns = {
        'a1': a1,
        'a2': a2,
        'a3': a3,
        'g_scale': sample_between(3, 0.28, 0.36, rng, decimals=2),
        'stim_limit': sample_between(3, 20, 40, rng, decimals=1),
        'tau1': sample_between(3, 0.6, 1.0, rng, decimals=2) * 1e-3,
        'tau2': sample_between(3, 10, 16, rng, decimals=0) * 1e-3,
        'tau3': sample_between(3, 0.1, 0.3, rng, decimals=2) * 1e-3,
    }

    # Cardiac
    a1 = sample_between(3, 0.6, 0.8, rng, decimals=2)
    a2 = np.ones(3) - a1
    a3 = np.zeros(3)
    params_cns = {
        'a1': a1,
        ...
        'a3': a3,
        'g_scale': sample_between(3, 0.28, 0.36, rng, decimals=2),
        'stim_limit': sample_between(3, 14, 20, rng, decimals=1),
        'tau1': sample_between(3, 2, 3, rng, decimals=1) * 1e-3,
        'tau2': sample_between(3, 1, 2, rng, decimals=1) * 1e-3,
        'tau3': sample_between(3, 1, 2, rng, decimals=1) * 1e-3,
    }
